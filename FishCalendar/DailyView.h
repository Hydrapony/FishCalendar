...
#pragma mark 任务页代理
@protocol DailyViewDelegate

-(NSMutableArray*)addData; // 获取备选数据
-(bool)saveData:(NSDictionary*)dailyDic; // 保存新数据
-(void)updateList;// 更新列表
-(void)updateListAndProgressView:(long)addMinute; // 更新列表、进度

@end

@interface DailyView : UIScrollView <UITextFieldDelegate,UIPickerViewDataSource,UIPickerViewDelegate>{
...
@property(nonatomic,strong) id<DailyViewDelegate> dailyViewDelegate; // 任务页代理

- (instancetype)initWithDelegate:(id<DailyViewDelegate>)delegate;
