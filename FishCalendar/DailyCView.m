...
- (instancetype)initWithDelegate:(id<DailyCViewDelegate>)theDelegate andClassName:(NSString*)classname
{
    self = [super init];
    if (self) {
        self.dailyCViewDelegate = theDelegate;
        ...
        [self addControlWithClassName:classname]; // 新建任务，填充指定数据
    }
    return self;
}
...
// 加载备选数据
-(void)initData{
    NSMutableArray *adata = [self.dailyCViewDelegate addData];
...
// 新建任务，填充默认数据
-(void)addControlWithClassName:(NSString*)classname{
    int classNameArrayCount = (int)_classNameArray.count;
    for (int i = 0; i < classNameArrayCount; ++i) {
        ClassName *cn = _classNameArray[i]; // 默认Class
        if ([cn.name isEqualToString:classname]) {
            [_name selectRow:i inComponent:0 animated:NO];
    ...
    ClassName *cn = _classNameArray[0];
    _nameE.text = cn.homeworkname;
    _nameimg.image = [UIImage imageNamed:cn.img];
    _iconCheck = (int)[_iconArray indexOfObject:cn.img];
    [_enddate selectRow:15 inComponent:0 animated:NO];
...
-(void)save{
    ...
    if(![self.dailyCViewDelegate saveData:(NSDictionary*)dailyDic]){
        // 保存报错，数据错误
        [Constant alertError:(UIViewController*)_dailyCViewDelegate];
        return;
    }
    ...
    if ([nowDate0 compare:_dateenddate] != NSOrderedAscending) {
        [_dailyCViewDelegate updateSumTask:1];
    }
...
-(void)saveDaily{
    ...
    NSDictionary *dailyLDic = @{@"saveType":@"DailyLong",
                                ...
                                @"imgname":_iconArray[_iconCheck],
                                @"havedone":@(havedoneAll),
                                @"recentdate":[formatter stringFromDate:[[NSDate alloc] initWithTimeInterval:60*60*36 sinceDate:_dateenddate]], // 加1天零12小时，防止时区问题，不必精确因为enddate只保存日期 加一天是因为当天的直接在下面生成出来
                                @"fromdate":[formatter stringFromDate:[[NSDate alloc] initWithTimeInterval:3600*12 sinceDate:_dateenddate]],
                                @"info":(_isInfoCheckbox.selected?[_infoE text]:_nameE.text)};
    if(![self.dailyCViewDelegate saveData:(NSDictionary*)dailyLDic]){
        // 保存报错，数据错误
        [Constant alertError:(UIViewController*)_dailyCViewDelegate];
        return;
    }
